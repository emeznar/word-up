...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Otherwise, it is not.
            var theAnswer = response.count;
            if (response.count > 0){
                theAnswer = true;
                //console.log("You have a word!");
            }else{
                theAnswer = false;
...
function render() {
    ...
    // DONE 3 Give focus to the textbox.
    $("#textbox").focus();
    ...
    if (disallowedLetters.length > 0) {
        ...
        // DONE 8 append the red letter chips to the form
        $("#game").append(redLetterChips);
    }

    // if the game is over
    var gameOver = model.secondsRemaining <= 0;
    if (gameOver) {
        // DONE 9 disable the text box and clear its contents
 ...
 */
 function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        var x = wordScore(wordSubmission.word);
        if (wordSubmission.isRealWord === "true") {
            scoreChip = $("<span></span>").text(x);
            scoreChip.addClass("tag tag-success");
        }else{
            scoreChip = $("<span></span>").text("X");
...
/**
 * Given a word, returns a list of all the disallowed letters in that
...
function containsOnlyAllowedLetters(word) {
    ...
    var n = model.currentAttempt.includes(disallowedLettersInWord(word));
    if (n === true){
        return true;
    }else{
        return false;
 ...
 */
 ...
 function wordScore(word) {
    ...
    var wordStr = word.toString();//I added this
    var letters = wordStr.split("");//I changed this to use the string above
    ...
    var letterScores = [];
    //var wordScores = letters.map(letterScore);

    var summed = 0;
    var x = letters.map(letterScore);
    for (var key in x) {
     summed += x[key];
     }
...
function currentScore() {
    ...
    // DONE 20
    //console.log("here are wordScores");
    //console.log(wordScores);
    // return the total sum of the word scores
    var total = wordScores.reduce(function(sum, word) {
        total =  sum + word;
        return total;
    }, 0);
